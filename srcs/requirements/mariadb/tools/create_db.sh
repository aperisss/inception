...
if [ -f "/var/lib/mysql/entry" ];then #check if the database has been initialized before or not 
    ...
    mysql -e "CREATE USER '$DB_ROOT'@'%' IDENTIFIED BY '$DB_ROOT_PASSWORD'; GRANT ALL PRIVILEGES ON $DB_NAME.* TO '$DB_ROOT'@'%' IDENTIFIED BY '$DB_ROOT_PASSWORD' WITH GRANT OPTION; flush privileges;" 
